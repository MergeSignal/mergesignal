...
- **API:** `GET /scan/:id` on the API returns the scan row, including `result` (JSON) when status is `done`. Use an org-scoped API key when your deployment requires it (`Authorization` / app conventions in [DEPLOYMENT.md](../../DEPLOYMENT.md)).
