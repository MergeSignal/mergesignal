...
The composite uses an internal checkout path `_ms_rt` and a standard Node/pnpm build to produce the CLI; that layout may change in Phase 2 if the composite switches to a lighter prebuilt CLI delivery. **Do not** document internal directory names or build commands in user-facing pages (Getting started, README “Recommended”); keep them here or in code only.
